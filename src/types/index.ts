...
export interface Schedule {
  ...
  cancelReason?: string
  /**
   * 넣은 사람 이름 — 넣는 **순간** 서버가 그 줄에 적어 둔 값 (0077).
   *
   *  ⚠ 기사 계정은 남의 프로필을 못 읽습니다(RLS). 화면이 이름을 쓰려고
   *    프로필 읽기 권한을 열면 그 순간 남의 개인정보를 전부 읽게 됩니다.
...
export interface MaterialSupply {
  ...
  requestId?: string | null
  /**
   * 시연 세션 중 만들어진 기록이면 그 세션 id (0122에 읽기만 추가).
   *
   *  ⚠ 칸은 처음부터 있었습니다(`materials.demo_session_id`) — 수거 완료가
   *    자재를 함께 넣을 때 서버가 같은 표식을 붙입니다. 화면에서 안 읽고
   *    있었을 뿐입니다. Pilot 집계가 수거 쪽과 **같은 기준으로** 시연을
   *    빼려면 이 값이 필요합니다.
   */
  demoSessionId?: string | null
}
