...
// 개발 현황 — 고객에게 공개해도 자연스러운 표현으로 운영관리 고도화 근거를 정리
const TECH_DETAILS: { icon: LucideIcon; title: string; desc: string }[] = [
  ...
  { icon: Layers, title: '운영관리 화면 구축', desc: '거래처·배차·수거이력·자재 요청을 한 흐름에서 확인하는 관리 화면을 구축하고 있습니다.' },
      ...
      <main id="top">
        ...
        <div className="relative overflow-hidden bg-white text-navy-900">
          <div
            ...
            style={{
              backgroundImage:
                'linear-gradient(to right, rgba(20,184,166,0.10) 1px, transparent 1px), linear-gradient(to bottom, rgba(20,184,166,0.10) 1px, transparent 1px)',
              backgroundSize: '38px 38px',
              maskImage: 'radial-gradient(ellipse 80% 70% at 70% 0%, black 18%, transparent 72%)',
              WebkitMaskImage: 'radial-gradient(ellipse 80% 70% at 70% 0%, black 18%, transparent 72%)',
            }}
          />
          <Section id="tech" className="relative">
            <Reveal>
              <div className="grid grid-cols-1 gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
                <div>
                  ...
                  <p className="mt-4 text-[16px] leading-relaxed text-navy-600 sm:text-[17px]">
                  비원미래는 실제 수거·운반 현장에서 발생하는 거래처 정보, 수거조건, 차량 운행, 자재 요청, 수거이력을
                  체계적으로 관리하기 위한 운영관리 시스템을 개발하고 있습니다.
                  </p>
                </div>

                <div className="rounded-2xl bg-white p-4 shadow-[0_24px_80px_-28px_rgba(15,26,46,0.34)] ring-1 ring-navy-100">
                  <div className="flex items-center justify-between border-b border-navy-100 pb-3">
                    <div className="flex items-center gap-2.5">
                      <BrandMark size={34} />
                      <div>
                        <p className="text-[15px] font-extrabold text-navy-900">BEONE MIRAE</p>
                        <p className="text-[12px] font-bold text-navy-400">운영관리 시스템</p>
                      </div>
                    </div>
                    <span className="rounded-full bg-teal-50 px-3 py-1 text-[12px] font-bold text-teal-700">구축 중</span>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {[
                      ...
                      <div key={label} className="rounded-xl bg-slate-50 p-3 ring-1 ring-navy-100">
                        <p className="text-[12px] font-bold text-navy-400">{label}</p>
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-[0.9fr_1.1fr]">
                    <div className="rounded-xl bg-slate-50 p-4 ring-1 ring-navy-100">
                      <p className="text-[14px] font-extrabold text-navy-900">배차 보드</p>
                      <div className="mt-3 space-y-2">
                        {[
                          ...
                          <div key={place} className="flex items-center gap-2 rounded-lg bg-white px-3 py-2 ring-1 ring-navy-100">
                            <span className="text-[12px] font-bold text-navy-400">{time}</span>
                            <span className="min-w-0 flex-1 truncate text-[13px] font-bold text-navy-800">{place}</span>
                            <span className={`rounded-md px-2 py-0.5 text-[11px] font-bold ${tag === '일회용기저귀' ? 'bg-amber-50 text-amber-700' : 'bg-teal-50 text-teal-700'}`}>
                              {tag}
                            </span>
                          </div>
                        ))}
                    ...
                    <div className="rounded-xl bg-slate-50 p-4 ring-1 ring-navy-100">
                      <p className="text-[14px] font-extrabold text-navy-900">수거 이력</p>
                      <div className="mt-3 space-y-2">
                        {[
                          ['의료폐기물', '250.8 kg', '완료'],
                          ['일회용기저귀', '62.6 kg', '완료'],
                          ['자재 요청', '전용 용기 24개', '확인'],
                        ].map(([type, amount, state]) => (
                          <div key={type} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-lg bg-white px-3 py-2 text-[13px] ring-1 ring-navy-100">
                            <span className="font-bold text-navy-800">{type}</span>
                            <span className="font-semibold text-navy-500">{amount}</span>
                            <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[11px] font-bold text-emerald-700">{state}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <Reveal delay={0.05}>
              <div className="mt-8 flex flex-col items-start gap-4 rounded-2xl bg-slate-50 p-5 ring-1 ring-navy-100 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <button
                    type="button"
                    onClick={goSystem}
                    className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-teal-600 px-5 py-3 text-[15px] font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-teal-700"
                  >
                    운영관리 화면 보기 <ArrowRight size={17} strokeWidth={2.4} />
                  </button>
                  <button
                    type="button"
                    onClick={() => setTechOpen((v) => !v)}
                    aria-expanded={techOpen}
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-5 py-3 text-[15px] font-bold text-navy-700 ring-1 ring-navy-100 transition hover:bg-navy-50"
                  >
                    {techOpen ? '근거 접기' : '개발 근거 보기'}
                    <ChevronDown size={17} strokeWidth={2.4} className={`transition-transform ${techOpen ? 'rotate-180' : ''}`} />
                  </button>
                </div>
              </div>
            </Reveal>
                ...
                >
                  <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {TECH_DETAILS.map((d) => {
                      const Icon = d.icon
                      return (
                        <div key={d.title} className="rounded-xl bg-white p-6 shadow-card ring-1 ring-navy-100">
                          <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-teal-50 text-teal-700">
                            <Icon size={22} strokeWidth={2.2} />
                          </span>
                          <p className="mt-4 text-lg font-bold text-navy-900">{d.title}</p>
                          <p className="mt-2 text-[15px] leading-relaxed text-navy-600">{d.desc}</p>
                        </div>
                      )
                    })}
                  </div>
