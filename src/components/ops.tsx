  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[1.07rem] font-bold text-navy-800">{SEPARATION_NOTICE.title}</p>
      </div>
      ...
      <p className="mt-3 rounded-2xl bg-navy-50 px-3.5 py-2.5 text-[0.98rem] font-semibold text-navy-600">
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">차량 정보 관리</p>
          <p className="text-[0.95rem] font-medium text-navy-400">특허 구성요소 130 · 폐기물 종류별 차량 분리</p>
        </div>
      </div>
            ...
            <div key={v.id} className="rounded-2xl bg-navy-50 p-3.5">
              <div className="flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="break-keep font-bold text-navy-800">{v.name}</span>
                </div>
                <span className="shrink-0 whitespace-nowrap text-[1.08rem] font-bold text-teal-600">
                  적재율 {p?.loadRate ?? 0}%
                </span>
              </div>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[0.98rem] font-medium text-navy-500">
      ...
      <p className="mt-3 text-[0.98rem] leading-snug text-navy-400">
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">처리장 / 소각장 정보</p>
          <p className="text-[0.95rem] font-medium text-navy-400">특허 구성요소 140 · 인계 가능 시간</p>
        </div>
      </div>
      <div className="mt-3 space-y-2.5">
        {FACILITIES.map((f) => {
          ...
          return (
            <div key={f.id} className="rounded-2xl bg-navy-50 p-3.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="flex shrink-0 items-center gap-1 text-[1.08rem] font-bold text-navy-600">
              ...
              <p className="mt-1.5 text-[0.98rem] text-navy-400">오늘 인계 예정: {names.join(', ') || '-'}</p>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">자재 소진 위험</p>
          <p className="text-[0.95rem] font-medium text-navy-400">입고·출고·추가요청 기반 시뮬레이션</p>
        </div>
      </div>
      {risks.length === 0 ? (
        <p className="mt-3 text-[1.08rem] text-navy-400">현재 소진 위험으로 표시된 거래처가 없습니다.</p>
      ) : (
        <ul className="mt-3 space-y-2">
          {risks.map((r) => (
            <li key={r.clientId} className="rounded-2xl bg-navy-50 p-3.5">
              <div className="flex items-center justify-between gap-2">
                <span className="break-keep font-bold text-navy-800">{r.clientName}</span>
                <RiskBadge level={r.level} />
              </div>
              <p className="mt-1 text-[1.08rem] text-navy-500">
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">격리 / 긴급 수거 확인</p>
            ...
            <li key={a.clientId} className="flex items-start gap-2.5 rounded-2xl bg-navy-50 p-3.5">
              ...
              <div className="min-w-0">
                <p className="font-bold text-navy-800">
                  {a.clientName} <span className="ml-1 text-[0.98rem] font-bold text-rose-500">{a.kind}</span>
                </p>
                <p className="text-[1.08rem] text-navy-500">{a.message}</p>
              </div>
            </li>
          )
        })}
      ...
      <div className="mt-3 space-y-1">
        {ISOLATION_NOTICE.map((n) => (
          <p key={n} className="text-[0.98rem] leading-snug text-navy-400">· {n}</p>
        ))}
      </div>
    </div>
  )
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">특허 구성요소 매핑</p>
          <p className="text-[0.95rem] font-medium text-navy-400">
        ...
        {PATENT.components.map((c) => (
          <div key={c.code} className="flex flex-col gap-1 rounded-2xl bg-navy-50 p-3.5 sm:flex-row sm:items-center sm:gap-3">
            <div className="flex shrink-0 items-center gap-2 sm:w-52">
              <span className="rounded-lg bg-white px-2 py-0.5 text-[0.95rem] font-extrabold text-navy-400">{c.code}</span>
              <span className="text-[1.08rem] font-bold text-navy-800">{c.name}</span>
            </div>
            <span className="text-[1.08rem] text-navy-500">{c.feature}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[1.07rem] font-bold text-navy-800">사업계획서와 MVP 정합성</p>
          <p className="text-[0.95rem] font-medium text-navy-400">사업계획서 방향 ↔ 앱 기능</p>
        </div>
      </div>
      <div className="mt-3 space-y-2">
        {BUSINESS_PLAN.map((b) => (
...
export function OfficeSavingsCard() {
  return (
    <div className="card p-5">
      <p className="text-[1.07rem] font-bold text-navy-800">이 프로그램이 줄여주는 사무업무</p>
      <p className="mt-0.5 text-[0.95rem] font-medium text-navy-400">관리 부담 완화 · 효과는 실증 후 검증 예정</p>
      <ul className="mt-3 grid gap-2 sm:grid-cols-2">
        {OFFICE_SAVINGS.map((t) => (
          <li key={t} className="flex items-start gap-2 text-[1.08rem] font-medium text-navy-600">
...
export function FuturePlanCard() {
  return (
    <div className="card p-5">
      <p className="text-[1.07rem] font-bold text-navy-800">향후 고도화</p>
      <ul className="mt-3 flex flex-wrap gap-2">
        {FUTURE_PLAN.map((t) => (
          <li key={t} className="rounded-full bg-navy-50 px-3 py-1.5 text-[0.98rem] font-bold text-navy-600">
            {t}
          </li>
        ))}
      </ul>
