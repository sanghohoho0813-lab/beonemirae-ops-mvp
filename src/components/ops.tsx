...
export function SeparationNotice() {
  return (
    <div className="card p-5">
      ...
      <p className="mt-3 rounded-2xl bg-navy-50 px-3.5 py-2.5 text-[0.85rem] font-semibold text-navy-600">
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">특허 구성요소 130 · 폐기물 종류별 차량 분리</p>
        </div>
      </div>
          ...
          return (
            <div key={v.id} className="rounded-2xl bg-navy-50 p-3.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="shrink-0 whitespace-nowrap text-[0.95rem] font-bold text-teal-600">
                  적재율 {p?.loadRate ?? 0}%
                </span>
              </div>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[0.85rem] font-medium text-navy-500">
      ...
      <p className="mt-3 text-[0.85rem] leading-snug text-navy-400">
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">특허 구성요소 140 · 인계 가능 시간</p>
        </div>
      </div>
      <div className="mt-3 space-y-2.5">
        {FACILITIES.map((f) => {
          ...
          return (
            <div key={f.id} className="rounded-2xl bg-navy-50 p-3.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="flex shrink-0 items-center gap-1 text-[0.95rem] font-bold text-navy-600">
              ...
              <p className="mt-1.5 text-[0.85rem] text-navy-400">오늘 인계 예정: {names.join(', ') || '-'}</p>
            </div>
          )
        })}
      </div>
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">입고·출고·추가요청 기반 시뮬레이션</p>
        </div>
      </div>
      {risks.length === 0 ? (
        <p className="mt-3 text-[0.95rem] text-navy-400">현재 소진 위험으로 표시된 거래처가 없습니다.</p>
      ) : (
        <ul className="mt-3 space-y-2">
          {risks.map((r) => (
            <li key={r.clientId} className="rounded-2xl bg-navy-50 p-3.5">
              ...
              <p className="mt-1 text-[0.95rem] text-navy-500">
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">격리의료폐기물 보관기한 대응</p>
        </div>
      </div>
      <ul className="mt-3 space-y-2">
        {alerts.map((a) => {
            ...
            <li key={a.clientId} className="flex items-start gap-2.5 rounded-2xl bg-navy-50 p-3.5">
              ...
              <div className="min-w-0">
                <p className="font-bold text-navy-800">
                  {a.clientName} <span className="ml-1 text-[0.85rem] font-bold text-rose-500">{a.kind}</span>
                </p>
                <p className="text-[0.95rem] text-navy-500">{a.message}</p>
              </div>
            </li>
          )
        })}
      ...
      <div className="mt-3 space-y-1">
        {ISOLATION_NOTICE.map((n) => (
          <p key={n} className="text-[0.85rem] leading-snug text-navy-400">· {n}</p>
        ))}
      </div>
    </div>
  )
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">
        ...
        {PATENT.components.map((c) => (
          <div key={c.code} className="flex flex-col gap-1 rounded-2xl bg-navy-50 p-3.5 sm:flex-row sm:items-center sm:gap-3">
            <div className="flex shrink-0 items-center gap-2 sm:w-52">
              <span className="rounded-lg bg-white px-2 py-0.5 text-[0.82rem] font-extrabold text-navy-400">{c.code}</span>
              <span className="text-[0.95rem] font-bold text-navy-800">{c.name}</span>
            </div>
            <span className="text-[0.95rem] text-navy-500">{c.feature}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="card p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[0.82rem] font-medium text-navy-400">사업계획서 방향 ↔ 앱 기능</p>
        </div>
      </div>
      <div className="mt-3 space-y-2">
        {BUSINESS_PLAN.map((b) => (
          <div key={b.goal} className="flex items-center gap-2 rounded-2xl bg-navy-50 px-3.5 py-2.5">
            <span className="min-w-0 flex-1 text-[0.95rem] font-semibold text-navy-700">{b.goal}</span>
            <span className="shrink-0 text-navy-300">→</span>
            <span className="min-w-0 flex-1 text-right text-[0.95rem] font-bold text-teal-700">{b.feature}</span>
          </div>
        ))}
      </div>
    </div>
...
export function OfficeSavingsCard() {
  return (
    <div className="card p-5">
      ...
      <p className="mt-0.5 text-[0.82rem] font-medium text-navy-400">관리 부담 완화 · 효과는 실증 후 검증 예정</p>
      <ul className="mt-3 grid gap-2 sm:grid-cols-2">
        {OFFICE_SAVINGS.map((t) => (
          <li key={t} className="flex items-start gap-2 text-[0.95rem] font-medium text-navy-600">
  ...
  return (
    <div className="card p-5">
      ...
      <ul className="mt-3 flex flex-wrap gap-2">
        {FUTURE_PLAN.map((t) => (
          <li key={t} className="rounded-full bg-navy-50 px-3 py-1.5 text-[0.85rem] font-bold text-navy-600">
            {t}
          </li>
        ))}
      </ul>
