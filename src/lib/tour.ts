...
export interface TourStep {
  ...
  why?: string
  /**
   * **직접 눌러야 넘어가는 단계** — 여기 한 줄을 적으면 설명만 읽는 단계와
   * 확실히 갈립니다 (0109).
   *
   *  읽기만 하는 단계   투어가 화면을 덮고, 「다음」으로 넘어갑니다
   *  직접 누르는 단계   강조한 자리만 **뚫어 두고**, 실제로 누르면 넘어갑니다
   *
   *  ⚠ 심사 시연에서 제일 중요한 차이입니다. 「지금 제가 누르는 겁니까,
   *    그냥 보는 겁니까」를 헷갈리면 그 자리에서 흐름이 끊깁니다.
   *  ⚠ 이 값이 없으면 예전과 **똑같이** 동작합니다 — 기존 세 투어는
   *    한 글자도 바뀌지 않습니다.
   */
  hands?: string
}

export type TourId = 'staff' | 'field' | 'client' | 'demo'
  ...
  steps: [
    {
      ...
      title: '수거 요청',
      action: '「수거 요청」이나 「자재·용기 요청」을 누르고 내용만 적어 보내세요. 급하면 긴급으로 표시하실 수 있습니다.',
    ...
    {
      //  0083 — 문의. 「이건 어떻게 되나요」는 수거 요청과 다른 자리입니다.
      route: '/portal/support',
      title: '궁금한 것은 문의로',
      action: '수거 일정 · 자재 · 정산 중에 고르고, 제목과 내용을 적어 보내세요.',
      linked: '담당자가 확인하면 상태가 바뀌고, 답변이 이 화면에 그대로 표시됩니다.',
      result: '전화가 연결될 때까지 기다리지 않으셔도 되고, 나중에 다시 찾아볼 수 있습니다.',
    },
    {
      route: '/portal',
...
// ── D. 심사 시연 (60초) ──────────────────────────────────────────────────────
//
//  실사·심사 자리에서 대표님이 직접 돌리는 여섯 단계입니다. 기능을 늘어놓지
//  않고 **한 바퀴**만 보여 줍니다.
//
//      AX 가 시킨다 → 사람이 한다 → AX 가 확인한다
//
//  ⚠ 3·4단계는 **진짜로 누릅니다.** 4단계에서 저장하는 수거 기록은 시연용이
//    아니라 그날의 실제 기록입니다 — 그래서 6단계의 「확인했어요」가 뜹니다.
//    보여주기용 가짜 데이터를 만들지 않기 때문에 이 순서가 성립합니다.
//  ⚠ 다른 세 투어와 달리 STORY 마디(beats)를 달지 않습니다. 공통 스토리
//    열 마디를 다 덮는 투어가 아니라, 한 바퀴만 보여 주는 자리입니다.
//  ⚠ 역할 투어가 아니므로 `tourFor()` 는 이 투어를 절대 돌려주지 않습니다.
//    시연 화면(/presentation)에서 대표님이 직접 시작합니다.
const DEMO: Tour = {
  id: 'demo',
  label: '심사 시연 (60초)',
  minutes: '1분',
  intro: '오늘 할 일을 AX 가 고르고, 직접 처리하고, AX 가 그 기록을 확인하는 한 바퀴를 봅니다.',
  steps: [
    {
      route: '/',
      anchor: 'today-focus',
      title: '오늘 확인할 업무',
      action: '아침에 이 한 화면만 봅니다.',
      linked: '전화·카톡으로 받은 요청, 종이에 적은 수거량, 엑셀로 옮긴 정산이 여기 한 줄로 모입니다.',
      result: '무엇부터 해야 하는지 찾는 시간이 없어집니다.',
    },
    {
      route: '/ax-coach',
      anchor: 'coach-total',
      title: 'AX 코치 — 무엇이 비어 있는가',
      action: '업무 활용 · 매출 증거 · 운영 확장 · 병원 직접사용 — 네 갈래를 시스템이 스스로 셉니다.',
      linked: '아래 「오늘 이것만 해주세요」는 이 중 가장 비어 있는 곳을 채우는 일로 고릅니다.',
      result: '할 일이 세 가지로 줄어듭니다.',
      why: '이 숫자는 자료가 얼마나 모였는지이지, 성과가 그만큼 좋아졌다는 뜻이 아닙니다 — 화면에도 그렇게 적혀 있습니다.',
    },
    {
      route: '/ax-coach',
      anchor: 'coach-go',
      title: '추천에서 「수거 입력으로」',
      hands: '「수거 입력으로」 단추를 눌러 주세요',
      action: '추천을 읽고 끝내지 않고, 그 자리에서 업무 화면으로 갑니다.',
      linked: '누른 것은 「했다」가 아니라 「업무하러 갔다」는 기록입니다. 이 화면에는 완료 단추가 없습니다.',
      result: '누르면 바로 수거 입력 화면으로 넘어갑니다.',
    },
    {
      route: '/collection',
      anchor: 'collect-save',
      title: '수거 1건 입력',
      hands: '거래처·수거량을 적고 저장을 눌러 주세요',
      action: '현장에서 한 번만 적습니다.',
      linked: '지금 저장하는 것은 시연용이 아니라 오늘의 실제 기록입니다.',
      result: '저장하면 다음 단계가 저절로 열립니다.',
      why: '잘못 눌렀으면 저장 직후 그 자리에서 되돌릴 수 있고, 되돌린 것도 기록에 남습니다.',
    },
    {
      route: '/collection',
      anchor: 'collect-done',
      title: '한 번 입력이 여러 곳으로',
      action: '방금 저장한 한 건이 어디까지 갔는지 그대로 보여 줍니다.',
      linked: '오늘 일정 · 수거이력 · 거래처 최근활동 · 자재 재고 · 처리장 인계 · 수거대장 초안까지 함께 갱신됐습니다.',
      result: '사무실이 엑셀에 다시 옮겨 적을 것이 없습니다.',
      why: '틀렸으면 「되돌리기」로 원래대로 돌립니다 — 되돌린 사실도 감사기록에 남습니다.',
    },
    {
      route: '/ax-coach',
      anchor: 'coach-done',
      title: 'AX 가 실제 기록을 확인합니다',
      action: '코치로 돌아오면 「실제 업무기록을 확인했어요」가 떠 있습니다.',
      linked: '단추를 눌러서가 아니라, 방금 저장한 수거 기록이 실제로 생겼기 때문입니다.',
      result: '시킨 일이 정말 일어났는지 시스템이 스스로 확인합니다. 그래서 이 숫자는 만들어 낸 숫자가 아닙니다.',
    },
  ],
  finish: { label: 'AX 도입 성과 보기', to: '/performance' },
}

export const TOURS: Record<TourId, Tour> = { staff: STAFF, field: FIELD, client: CLIENT, demo: DEMO }
