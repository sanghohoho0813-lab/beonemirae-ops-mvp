...
export interface Tour {
  ...
  finish: TourFinish
  /**
   * **심사 시연 전용 — 작게, 조용히** (0110).
   *
   *  일반 사용법 투어는 「가르치는」 자리라 설명이 넉넉해야 합니다. 심사 시연은
   *  반대입니다 — 보는 사람은 **화면**을 보러 왔지 설명을 읽으러 온 것이
   *  아닙니다. 이 표시가 붙은 투어에서만
   *
   *   · 설명 상자를 좁고 낮게 줄이고 (30rem → 21rem, 높이도 화면의 절반까지)
   *   · 자리가 있으면 **옆에** 세워 본문을 덜 가리고
   *   · 화면의 군더더기(오른쪽 위 도구 줄 · 하단 탭 · PC 보기 띠)를 감춥니다.
   *
   *  기존 세 투어에는 이 표시가 없으므로 한 글자도 바뀌지 않습니다.
   */
  compact?: boolean
}
...
const DEMO: Tour = {
  ...
  minutes: '1분',
  compact: true,
  intro: 'AX 가 오늘 할 일을 고르고, 사람이 그 일을 하고, AX 가 실제 기록으로 확인하는 한 바퀴입니다.',
  //  ── 다섯 단계 · 한 화면에 문장 둘 ──────────────────────────────────────
  //
  //   0109 에서는 여섯 단계에 설명이 795자였습니다. 소리 내어 읽으면 120초 —
  //   60초 안에 들어갈 수가 없었습니다. **단계 수가 아니라 말이 길었습니다.**
  //
  //   그래서 이 투어에서는 `linked`·`why` 를 쓰지 않습니다. 한 단계에
  //   「지금 무엇을 하는가(action)」와 「그래서 무엇이 달라지는가(result)」
  //   둘뿐입니다. 나머지 설명은 대표님 육성이 맡습니다.
  //
  //   ⚠ 대시보드 단계(0109 의 ①)를 뺐습니다. 이 투어가 보여 줄 것은
  //     **시킨다 → 한다 → 확인한다** 한 바퀴이고, 대시보드는 그 바퀴 밖입니다.
  //     화면 이동이 한 번 줄어드는 만큼 시연이 짧고 매끄러워집니다.
  steps: [
    {
      route: '/ax-coach',
      anchor: 'coach-total',
      title: 'AX 코치 — 무엇이 비어 있는가',
      action: '네 갈래 중 가장 비어 있는 곳을 시스템이 고릅니다.',
      result: '오늘 할 일이 세 가지로 줄어듭니다.',
    },
    {
      route: '/ax-coach',
      anchor: 'coach-go',
      title: '추천에서 바로 업무로',
      hands: '「수거 입력으로」를 눌러 주세요',
      action: '이 화면에는 「완료」 단추가 없습니다.',
      result: '누르면 수거 입력 화면으로 갑니다.',
    },
    {
      route: '/collection',
      anchor: 'collect-save',
      ...
      result: '오늘의 실제 기록으로 저장됩니다.',
    },
    {
      route: '/collection',
      anchor: 'collect-done',
      title: '한 번 입력이 여러 곳으로',
      action: '오늘 일정 · 수거이력 · 거래처 · 재고 · 인계까지 함께 갱신됐습니다.',
    ...
    },
    {
      ...
      action: '단추를 눌러서가 아니라, 기록이 실제로 생겼기 때문입니다.',
      result: '그래서 이 숫자는 만들어 낸 숫자가 아닙니다.',
    },
  ],
