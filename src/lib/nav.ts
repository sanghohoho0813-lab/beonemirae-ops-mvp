 ...
 *  순서는 실제 월 업무 흐름입니다: 편성 → 배차 → 자재 → 청구 → 점검 →
 *  미수 → 통장 → 이력 → 통계 → 계획.
 */
export const TOOL_NAV: NavItem[] = [
  ...
  { to: '/materials', label: '자재 관리', icon: Boxes, desc: '박스 · 비닐 · 바늘통 공급 내역', tone: 'navy' },
  ...
  { to: '/history', label: '수거이력', icon: History, desc: '전체 수거 입력 이력 · 감사기록', tone: 'navy' },
