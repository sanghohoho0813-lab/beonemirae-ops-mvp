-- 비원미래 운영 DB — 42차 (0056)
-- 담당 기사 배정 · 계정 사전 등록(초대) · 계정에 차량 묶기.
-- RUN_1~41 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0056_driver_assignment.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();   → 56
--              select public.app_health_check();      → ok: true
--
-- ────────────────────────────────────────────────────────────────────────────
-- 왜 필요한가
--
--   ① 기사님마다 본인이 맡은 거래처만 보이게
--      거래처가 50곳 가까이 되는데 지금은 기사 계정에 전부 보입니다. 병원
--      상호·주소·담당자 연락처가 담당이 아닌 사람 화면에도 다 뜹니다.
--      거래처 화면에서 담당 기사를 눌러 고르면 그 사람에게만 보입니다.
--
--   ② 계정을 대표님이 미리 만들어 두기
--      지금은 본인이 가입 신청을 하고 대표님이 승인을 눌러야 씁니다.
--      이메일·역할·차량·담당 거래처를 미리 적어 두면, 그 주소로 가입하는
--      순간 그대로 붙고 승인도 자동입니다.
--      **비밀번호는 이 표에 담지 않습니다** — 본인이 가입 화면에서 정합니다.
--
--   ③ 계정에 차량 묶기
--      기사님이 수거 입력에서 매번 「몇 호차 · 이름」을 고르지 않아도 됩니다.
--      묶어 두면 그 칸이 사라지고, 기록에 남는 이름은 로그인한 본인입니다.
--
-- ────────────────────────────────────────────────────────────────────────────
-- 가장 조심한 것 — 기사님 화면이 텅 비는 일
--
--   배정을 켰는데 아무것도 안 보이면 그날 수거가 통째로 멈춥니다. 그래서
--
--     · 배정이 **한 건도 없는** 기사 → 지금까지처럼 **전부** 보입니다
--     · 배정이 **한 건이라도 있는** 기사 → 그 목록만 보입니다
--
--   지금 쓰고 계신 계정들은 배정이 하나도 없으므로 **오늘과 똑같습니다.**
--   대표님이 거래처에서 기사를 고르기 시작하는 그 순간부터 좁아집니다.
--   배정을 다 지우면 원래대로 돌아옵니다.
--
-- 기존 데이터에 미치는 영향
--   · 표 두 개(client_assignments · staff_invites)가 새로 생깁니다. 비어 있습니다.
--   · profiles 에 칸 하나(vehicle_id)가 늘어납니다. 전부 null 입니다.
--   · **돈을 담은 표는 하나도 안 건드립니다.** 청구·입금·단가 그대로입니다.
--   · RLS 는 **좁히는 쪽으로만** 바뀝니다(현장 계정의 거래처·일정 읽기).
--     관리자·사무실은 한 줄도 안 좁아지고, 병원 계정은 원래대로 자기 병원만
--     봅니다(0006 그대로).
--   · 가입 트리거(handle_new_user)가 바뀝니다. **초대가 없으면 지금까지와
--     똑같이** 승인 대기입니다.
--   · 두 번 실행해도 안전합니다.
--
-- 실행 순서
--   1) 이 파일을 Supabase SQL Editor 에 통째로 붙여 넣고 실행
--   2) select public.app_schema_version();  → 56 이 나오면 끝
--   3) 사용자 관리 화면에서 기사 계정에 차량을 묶고,
--      거래처 화면에서 담당 기사를 고르면 그때부터 적용됩니다
