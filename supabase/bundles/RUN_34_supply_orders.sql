-- 비원미래 운영 DB — 34차 (0048)
-- 병원 소모품 주문 — 「다음 수거 때 같이 가져다 주세요」. RUN_1~33 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0048_supply_orders.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();  → 48 이 나와야 합니다.
--              select public.app_health_check();     → ok: true
--
-- 무엇이 생기나
--   · products              — 파는 물건 (사무실 재고 칸을 가리켜 이중 등록을 막습니다)
--   · product_orders        — 요청 → 확인 → 준비 → 전달예정 → 전달완료
--   · product_order_items   — 주문 줄 (단가·원가를 그때 값으로 박아 둡니다)
--   · request_product_order / set_product_order_status / upsert_product / product_sales_summary
--
-- 기존 데이터에 미치는 영향
--   · 표를 새로 만들 뿐, 기존 표는 건드리지 않습니다.
--   · office_stock 은 「전달완료」를 누를 때만 줄어듭니다. 실행만으로는 안 바뀝니다.
--   · 상품이 아직 하나도 없으므로 실행 직후에는 주문할 것이 없습니다 —
--     관리자가 상품을 등록하면 그때부터 병원이 요청할 수 있습니다.
--
-- 방어 (격리 DB 61건으로 확인)
--   · 요청만으로 재고가 빠지지 않습니다 — 전달완료에서 한 번만
--   · 네 사람이 같은 순간에 전달완료를 눌러도 재고는 한 번만 빠집니다
--   · 상품가가 바뀌어도 이미 올린 주문 금액은 안 바뀝니다
--   · 병원은 다른 병원의 주문·수량·금액을 볼 수 없습니다 (RLS)
--   · 확정된 판매금액을 표에서 직접 고칠 수 없습니다
