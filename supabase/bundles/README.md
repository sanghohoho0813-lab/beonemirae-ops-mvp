...
| 10 | `RUN_10_import_new_items.sql` | 0024 엑셀 가져오기 — 실제 거래처 10곳 규격(35L·79L 박스 등) 합산 + 미래 날짜 거부 |
| 11 | `RUN_11_monthly_actuals.sql` | 0025 엑셀 월 실적 — 날짜 없는 달의 수거량·매출을 월 단위로 보관 |
| 12 | `RUN_12_payment_receipts.sql` | 0026 부분입금 — 청구 1건에 입금 여러 건 (입금일·입금액·결제수단) |
| 13 | `RUN_13_kst_date_guards.sql` | 0027 「오늘」을 한국 시각으로 — 새벽에 오늘 기록이 거부되던 문제 |
...
## 10차 — 엑셀 가져오기 실측 반영 (0024)

이사님에게 받은 **실제 거래처 관리 엑셀 10개**(+더원 = 11개)를 전수 분석해
반영했습니다. 테이블·권한 변경은 없고 가져오기 함수 본문만 바뀝니다.

| 반영한 것 | 근거 |
|---|---|
| 새 규격 합산 — 35L·79L 박스, 10L 합성수지, 12L 봉투형용기, 기저귀박스(중) | 오남한양·엠에스·서울본브릿지·남양주백·목동현대웰 파일에 실재 |
| 미래 날짜 거부 | 서울인화 「2025년 12월~」 명세서에 날짜가 2026-12 로 밀려 적혀 있었음 — 그대로 넣으면 미래의 수거가 만들어짐 |

앱 쪽(같이 배포되는 코드)은 거래처별 정산방식을 함께 배웁니다 —
kg 단가 · **월정액**(오남한양 900만 / 해올 130만+300만) · **박스 개당**
(서울온케어 8,000 / 서울인화 10,000 / 삼성 18,000) · **부가세 별도**
(목동현대웰 10%). 전부 실제 파일의 값이며, 가져오기가 자동으로 채웁니다.


## 11차 — 엑셀 월 실적 (0025)

엑셀에는 **날짜별 수거**(거래명세서)와 **월 합계**(정산금 세부내역) 두 가지가
있습니다. 지금까지는 앞의 것만 옮겼습니다 — 뒤의 것은 날짜를 알 수 없어
수거 기록으로 만들 수 없기 때문입니다(없는 수거일을 지어내면 안 됩니다).

그 결과 **오남한양병원·남양주백병원처럼 명세서에 날짜가 없는 거래처는
8개월치 실적이 엑셀에 멀쩡히 있는데도 화면이 전부 비었습니다** — 월평균
수거량 0kg, 월 정산 없음, 월간 리포트 없음, 미수금 없음. 날짜가 있는
거래처도 명세서는 최근 한 달치뿐이라 나머지 달은 똑같이 비었습니다.

이제 월 합계를 **월 합계 그대로** 보관합니다(`client_monthly_actuals`).
날짜별 수거로 바꾸지 않고, 화면에서도 「엑셀에서 가져온 월 실적」으로
구분해 보여 줍니다 — 근거가 다른 두 숫자를 섞지 않기 위해서입니다.

| 채워지는 곳 | |
|---|---|
| 월평균 수거량 | 달 단위로 셉니다. 수거 기록이 있는 달은 그 합계, 없는 달은 엑셀 월 실적 |
| 월 정산·명세서 | 「엑셀에서 가져온 월 실적」 표 (달·수거량·매출·출처 파일) |
| 월간 리포트 | 같은 표 + 왜 리포트를 못 만드는지 |
| 결제·미수금 | 같은 표 + **미수금은 만들지 않습니다** (어느 달이 입금됐는지 파일에 없음) |

같은 거래처·같은 달이 이미 있으면 값이 같을 때만 지나가고 다르면 충돌로
셉니다 — 나중에 올린 파일이 앞의 기록을 조용히 바꾸지 않습니다.

확인:

```
node scratchpad/xl/db_e2e.mjs    # 120건 (월 실적 저장·중복·충돌·월 표기 검증 포함)
```


## 12차 — 부분입금 (0026)

지금까지 입금은 「입금완료 / 미수금」 두 상태뿐이었습니다. 100만원을 청구하고
30만원만 들어오면 표현할 방법이 없습니다 — 미수금으로 두면 받은 30만원이
사라지고, 입금완료로 두면 못 받은 70만원이 사라집니다. **둘 다 장부가
틀립니다.**

실제 거래처 파일에서 확인했습니다 — 서울인화 거래명세서에 「미납금액」 이월
줄이 있습니다. 이사님도 통장별 입금내역을 따로 엑셀로 정리하고 계십니다.

| 새로 되는 것 | |
|---|---|
| 입금 여러 건 | 한 청구에 입금일·입금액·결제수단·메모를 건별로 |
| 결제수단 | 계좌이체 · 카드 · 현금 · 기타 (카드결제 거래처가 실재) |
| 남은 미수금 | 청구액 − 누적입금. 「부분입금」 상태로 따로 표시 |
| 초과 입금 차단 | 청구액을 넘으면 거부 — 0 하나 더 치면 미수금이 마이너스가 됩니다 |
| 되돌리기 | 잘못 넣은 입금을 지우면 청구 상태도 함께 되돌아감 |
| 감사기록 | 입금·삭제 모두 남습니다 (돈 기록) |

**기존 데이터는 손대지 않습니다.** 이미 「입금완료」인 청구는 입금 기록이
없는데, 그런 청구는 전액 받은 것으로 봅니다. 과거 기록을 고치지 않으면서
새 방식이 함께 동작합니다.

권한: 사무실·관리자만. 현장 담당자는 RLS 로 **읽지도 못합니다.**

확인:

```
node scratchpad/xl/db_receipts.mjs   # 25건 (부분입금·초과차단·되돌리기·권한·호환)
```


## 13차 — 「오늘」을 한국 시각으로 (0027)

가져오기·입금의 미래 날짜 가드가 `current_date` 를 썼습니다. Supabase 의 DB
시간대는 UTC 라, **한국 시각 00:00~09:00 사이에는 UTC 가 아직 어제**입니다.

그 9시간 동안:
- 오늘(한국) 수거한 기록을 엑셀로 넣으면 「아직 오지 않은 날짜」로 거부
- 오늘(한국) 들어온 입금을 기록해도 같은 이유로 거부

새벽에 전날 명세서를 정리하는 시간대라 실제로 걸리는 자리입니다.
이 프로젝트의 다른 함수들(`complete_collection` 등)은 이미
`(now() at time zone 'Asia/Seoul')::date` 를 쓰고 있었고 **가드만 빠져
있었습니다.** 같은 기준으로 맞췄습니다. 표·권한 변경은 없습니다.

실제로 재현·확인했습니다 — DB `current_date` 2026-08-12 / 한국 2026-08-13
상황에서 8/13 자 수거·입금이 정상 저장되고, 8/14(진짜 미래)는 여전히 거부.
