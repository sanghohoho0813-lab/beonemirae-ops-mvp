-- 비원미래 운영 DB — 45차 (0059)
-- 잡아 둔 방문을 옮기고 무를 수 있게. RUN_1~44 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0059_move_cancel_visit.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();   → 59
--              select public.app_health_check();      → ok: true
--
-- ────────────────────────────────────────────────────────────────────────────
-- 왜 필요한가 — 잡는 길만 있고 무르는 길이 없었습니다
--
--   RUN_44 로 날짜를 정해 방문을 **잡을** 수 있게 됐습니다. 그런데
--   병원이 「그날 말고 다음 주로 해 주세요」 하면 화면에서 할 수 있는 것이
--   없습니다. 잘못 잡은 방문도 지우는 길이 없어 그대로 남고, 그날 기사가
--   나갑니다.
--
...
--
-- 이 SQL 이 하는 일
--   ① schedules 에 canceled_at · cancel_reason 칸
--   ② move_visit(...)   — 날짜·시각·차량을 옮깁니다
--   ③ cancel_visit(...) — 무릅니다 (**지우지 않습니다**)
--   ④ 색인·함수 세 개를 「무른 것 제외」로 맞춥니다
--
-- ⚠ 왜 book_visit · create_planned_schedules 가 다시 나오는가
--
--   같은 날 중복을 막는 색인에 조건이 하나 붙었습니다. 그 색인을 쓰는
--   `on conflict (...) where ...` 는 **색인과 글자 하나까지 같아야** 합니다.
--   색인만 바꾸고 함수를 그대로 두면, SQL 을 실행한 그 순간부터
--   **방문 예약과 일정 편성이 통째로 죽습니다.** 그래서 두 함수를 원본
--   그대로 옮겨 온 뒤 그 한 줄만 고쳐 함께 넣었습니다.
--
-- 기존 데이터에 미치는 영향
--   · 표를 만들지도 지우지도 않습니다. 칸 둘만 늘립니다.
--   · 이미 있는 일정·수거·청구는 한 줄도 안 바뀝니다.
--   · 완료된 수거는 옮길 수도 무를 수도 없습니다 (서버가 막습니다).
--   · RLS 는 한 줄도 안 건드립니다.
--   · 두 번 실행해도 안전합니다.
--
-- 실행 순서
--   1) 이 파일을 Supabase SQL Editor 에 통째로 붙여 넣고 실행
--   2) select public.app_schema_version();  → 59 가 나오면 끝
