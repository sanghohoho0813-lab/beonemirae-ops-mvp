-- 비원미래 운영 DB — 33차 (0047)
-- 회사 자신에 대한 사실 — 직원 명부 · 국세청 신고 매출. RUN_1~32 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0047_company_facts.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();  → 47 이 나와야 합니다.
--              select public.app_health_check();     → ok: true
--              select name, position, waste_scope from public.staff order by insured_from;
--              select * from public.tax_filings order by period_from;
--
-- 넣는 값 (2026-03-19 발급 서류 기준)
--   · 직원 6명 — 송명근(대표) 홍현주(이사) · 현장 4명
--     의료폐기물: 오대성 · 김준기   |   일회용기저귀: 백광호 · 김진환
--   · 부가가치세 과세표준 7개 반기 (2023 상반기 ~ 2026 상반기)
--
-- ⚠ 주민등록번호는 넣지 않습니다. 담을 칸 자체를 만들지 않았습니다.
-- ⚠ 이미 같은 이름의 직원이 있으면 건드리지 않습니다 — 사람이 고친 값을
--    이 파일이 되돌려 놓지 않습니다.
