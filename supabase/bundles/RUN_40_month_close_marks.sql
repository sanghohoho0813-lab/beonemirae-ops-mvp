-- 비원미래 운영 DB — 40차 (0054)
-- 「보냈습니다 / 발행했습니다」를 기록할 수 있게. RUN_1~39 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0054_month_close_marks.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();   → 54
--              select public.app_health_check();      → ok: true
--
-- 왜 필요한가
--   월 마감 여섯 단계 중 「거래명세서 발송」과 「세금계산서 발행」은 시스템이
--   알 수 없습니다 — 뽑을 수 있다는 것까지만 압니다. 그래서 다 하고도 화면은
--   영원히 「준비됨」이었고, 어느 달을 빠뜨렸는지 기억으로 답해야 했습니다.
--   사람이 눌러 기록하면 그때부터 「끝」으로 봅니다.
--
-- 기존 데이터에 미치는 영향
--   · 표 하나가 새로 생깁니다(month_close_marks). 비어 있는 채로 시작합니다.
--   · 금액·청구·입금·재고·수거는 **하나도 바뀌지 않습니다.**
--   · 이 표에는 금액 칸이 아예 없습니다.
--   · 두 번 실행해도 안전합니다.
--
-- 실행 순서
--   1) 이 파일을 Supabase SQL Editor 에 통째로 붙여 넣고 실행
--   2) select public.app_schema_version();  → 54 가 나오면 끝
--   3) 앱을 새로고침하면 「월말 청구 → 월 마감 진행상황」 아래에 표시 칸이 생깁니다
