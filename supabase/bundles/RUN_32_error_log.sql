-- 비원미래 운영 DB — 32차 (0046)
-- 오류를 남깁니다 — 「왜 안 됐나」에 답할 수 있게. RUN_1~31 이 적용된 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0046_error_log.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();  → 46 이 나와야 합니다.
--              select public.app_health_check();     → ok: true
--
-- 실행하면 그때부터 오류가 쌓입니다. 설정 화면의 「최근 오류」에서 보실 수 있습니다.
-- 지난 일은 남아 있지 않습니다 — 없는 기록을 만들어 내지 않습니다.
