-- 비원미래 운영 DB — 35차 (0049)
-- 판매 실적에 「다시 산 곳」 더하기. RUN_1~34 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0049_sales_repeat.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();  → 49 가 나와야 합니다.
--              select public.app_health_check();     → ok: true
--
-- 왜 필요한가
--   매출·이익만 보면 이 사업이 되는지 알 수 없습니다. 한 번은 호의일 수 있고
--   **두 번째부터가 매출**입니다. 한 번씩만 팔리고 아무도 다시 안 사면 매출이
--   늘어도 모델이 틀린 것이고, 그것을 빨리 알아야 합니다.
--
-- 기존 데이터에 미치는 영향 — 없습니다
--   · 표를 만들지도, 고치지도, 지우지도 않습니다.
--   · 읽기 전용 함수 product_sales_summary 하나를 다시 만들 뿐입니다
--     (돌려주는 칸 두 개 추가: repeatClients · repeatOrders).
--   · 잠금·권한도 그대로입니다 — 판매 실적은 여전히 사무실·관리자만 봅니다.
--
-- 실행 순서
--   RUN_34(0048) 를 이미 실행하셨어야 합니다. 이 파일 하나만 실행하면 됩니다.
--
-- 실행 전에는 화면의 「실적」 탭에서 「다시 산 곳」이 「—」로 나오고
-- 「RUN_35 미실행」이라고 그대로 적힙니다. 0 곳이라고 거짓말하지 않습니다.
