-- 비원미래 운영 DB — 43차 (0057)
-- 전달완료한 소모품이 그 달 청구에 실리게. RUN_1~42 뒤에 실행합니다.
--
-- 이 파일은 supabase/migrations/0057_product_billing.sql 과 내용이 같습니다.
--
-- 실행 후 확인: select public.app_schema_version();   → 57
--              select public.app_health_check();      → ok: true
--
-- ────────────────────────────────────────────────────────────────────────────
-- 왜 필요한가 — 물건은 나갔는데 청구서에 없었습니다
--
--   병원이 소모품을 주문하고(0048), 기사님이 수거 가는 길에 전달하고,
--   전달완료를 누르면 재고까지 빠집니다. 거기까지는 됐습니다.
--
--   그런데 **그 판매가 월말 청구에 안 붙었습니다.** 청구서에는 수거와
--   용기만 담기고, 판 물건 값은 어디에도 없었습니다. 이사님이 따로 기억해
--   두었다가 손으로 붙이지 않으면 그대로 사라지는 돈입니다.
--
--   이번 판에서
--    · 정산·거래명세서·월말청구에 소모품 줄이 실립니다 (화면 쪽)
--    · 단가는 **주문 시점에 굳어 둔 값**입니다 — 오늘 상품가를 올려도
--      이미 전달한 물건의 청구액은 안 흔들립니다
--    · **전달완료한 주문만** 실립니다. 요청·확인·준비·전달예정은 아직
--      매출이 아니고, 취소한 주문은 영영 매출이 아닙니다
--    · 단가가 비어 있는 품목은 0원이라 청구에 안 실립니다 — 대신 화면이
--      「단가가 없어 청구에 안 실린 소모품」이라고 이름까지 적습니다
--
-- 이 SQL 이 하는 일 — 중복 차단
--
--   금액은 지금까지처럼 화면이 계산합니다. 서버가 맡는 것은 하나입니다.
--    ① 같은 주문이 든 청구를 **두 번 확정할 수 없습니다** (창 두 개·새로고침)
--    ② **전달완료가 아닌 주문**은 청구에 담을 수 없습니다
--       (물건도 안 갔는데 돈을 달라는 청구서가 나가면 안 됩니다)
--    ③ 소모품만 있는 달도 청구할 수 있습니다 — 지금은 수거·자재가 없으면
--       「담을 기록이 없습니다」로 막혀 청구가 아예 안 만들어집니다
--
-- 기존 데이터에 미치는 영향
--   · 표를 만들지도 지우지도 않습니다. 칸도 안 늘립니다.
--   · **이미 확정한 청구는 하나도 안 바뀝니다** (굳혀 둔 값 그대로).
--   · 옛 청구에는 소모품 목록이 없습니다 — 없으면 빈 목록으로 봅니다.
--   · RLS 는 한 줄도 안 건드립니다.
--   · 두 번 실행해도 안전합니다.
--
-- 실행 순서
--   1) 이 파일을 Supabase SQL Editor 에 통째로 붙여 넣고 실행
--   2) select public.app_schema_version();  → 57 이 나오면 끝
--   3) 소모품 단가가 비어 있으면 「소모품 → 상품」에서 넣어 주세요.
--      단가가 없으면 전달해도 청구에 안 실립니다 (화면이 알려 줍니다)
