...
> **비원미래 운영 프로젝트(`pyjdrubmfyyyuscppcza`)에는 적용 완료했습니다.**
> 2026-08-09 기준 전체 검증 144건 PASS / 0 FAIL, 연속 2회 동일.
> 아래는 다른 고객사에 같은 절차를 밟을 때를 위해 남겨 둡니다.
...
## 적용 후 실측 (비원미래, 2026-08-09)

| 검증 | 결과 |
|---|---|
| `05_live.mjs` — Auth · RLS · CRUD · 수거 트랜잭션 · 감사 · 멀티세션 · Demo/Live | 66 / 0 |
| `06_cross_client.mjs` — 병원 간 격리 · 권한 상승 차단 | 15 / 0 |
| `07_settlement.mjs` — 유상/무상 · 월 정산 · 거래명세서 | 28 / 0 |
| `08_browser_live.mjs` — 브라우저 PC(1440) 입력 → 모바일(390) 조회 | 35 / 0 |

`06` 의 「병원A → 소속 병원 바꿔치기 차단」이 **403** 으로 바뀌었습니다.
같은 정책이 고정하는 `email` 도 함께 막히고, 본인 이름·글자크기 수정은
그대로 됩니다.

### 적용 여부를 헷갈리지 않으려면

RUN_4 를 돌렸는데도 바꿔치기가 200 으로 통과한 적이 있었습니다.
그래서 `06` 이 이제 어느 세대의 정책이 걸려 있는지 직접 판별합니다.

- `email` 도 같이 바뀌면 → `0002`(구) 정책입니다. **RUN_4 가 적용되지 않았습니다.**
- `email` 은 막히는데 `client_id` 만 새면 → `0012` 는 들어갔는데 본문이 다릅니다.
...
where polrelid = 'public.profiles'::regclass
  and polname  = 'profiles_update_self';
```

출력에 `client_id` 와 `email` 이 있으면 적용된 것입니다.
