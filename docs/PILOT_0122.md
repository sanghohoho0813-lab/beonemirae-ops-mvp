...
| 전체 회귀 (142 스위트, **순차 실행**) | — | **검사 5,762 · 실패 10 — 10건 모두 기존 실패**(변경 전 커밋 `85c033d` 에서 같은 줄이 그대로 남). 새로 생긴 실패 0 |
| `tsc --noEmit` | — | 통과 |

### 전체 회귀 — 측정 방법과 결과 (2026-09-16)

**결과 (0123 최종 빌드): 142 스위트 · 검사 5,762 · 실패 10 · 새로 생긴 실패 0.**

| 남은 실패 | 건수 | 판정 |
|---|---|---|
| `check_tourclean` — 투어를 끝낸 뒤 `inert/aria-hidden` 이 1개 남음 (PC·폰 5가지 종료 방식) | 5 | **기존 실패** — 변경 전 커밋(`85c033d`)에서 같은 5줄이 그대로 남 |
| `check_materials82` — 공급 내역 목록이 0건 (「최근 30건」 · 「더 보기」 · 거래처 필터) | 5 | **기존 실패** — 변경 전 커밋(`85c033d`)에서 **12검사 · 5실패**로 같은 줄이 그대로 남. 이 스위트는 실행마다 진행되는 검사 수가 달라집니다(12~23건, 그때 실패는 5→1) |
...
- [x] 정산/청구/재고 코드 변경 0 · 회귀 142 스위트 · 검사 5,762 · 실패 10(전부 기존)
