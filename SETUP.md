...
| 무엇 | 지금 상태 | 왜 문제인가 |
|---|---|---|
| **청구 등록** | 미수금 화면은 조회·입금처리만 됩니다 | 월 정산은 계산되는데 그 결과를 청구로 만드는 버튼이 없어, 현재 청구는 DB 에 직접 넣어야 합니다 |
...
| **자재 화면의 공급이 재고를 차감하지 않습니다** | 수거 입력의 동시공급은 재고를 줄이고 원장에도 남지만, 자재 관리 화면의 공급 등록은 둘 다 하지 않습니다 | 자재 화면이 '이미 나간 것을 사후 기록'하는 용도인지, 아니면 차감해야 하는지 |

### 계정·운영

| 무엇 | 왜 |
|---|---|
| **관리자를 2명 이상 두세요** | `0016_admin_lockout_guard.sql` 을 적용하면 서버가 막지만, 관리자가 둘이면 한쪽이 잠겨도 다른 쪽으로 풀 수 있습니다 |
...
| **감사로그 화면은 최근 200건까지만 보입니다** | 서버에는 다 남아 있지만 화면에서는 그 이전을 열어 볼 수 없습니다. 지금 DB 에 510건이 있고 실사용에서는 하루 10~30건씩 쌓이므로, 화면으로 되짚을 수 있는 기간은 **약 1~2주**입니다. 더 오래 보려면 기간 선택이 필요합니다 |
