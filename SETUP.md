...
`--setup` 은 오늘 예정 일정 하나와 함께 **지난 3주치 수거 4건**(96·104·112·58kg)을
깔아 둡니다. 이력이 하나도 없는 거래처는 다음 수거일을 예측할 수 없어 추천이 나오지
않고, 그 달에 집계할 것이 없어 거래명세서 버튼도 잠깁니다 — 둘 다 맞는 동작이지만
그 상태에서는 검사가 밟을 길이 없습니다. 이 기록도 `[검증]` 거래처 것이라
`--cleanup` 이 함께 지웁니다.
