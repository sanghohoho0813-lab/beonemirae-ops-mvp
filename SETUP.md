...
정산 금액이 나와도 그것을 **받을 돈으로 옮기는 길이 화면에 없습니다.**
그래서 미수금 화면은 비어 있고, 대시보드의 미수금 합계도 0원으로 나옵니다
(실제로는 0원이 아닌데도 그렇게 보입니다).

엑셀에는 이 자리가 `AE열 정산완료 / 미수금` 으로 있습니다. 즉 **엑셀의
'정산금 세부내역'과 '거래명세서'는 시스템이 대체할 수 있지만, '미수금
상태'는 대체하지 못합니다.** 이사님이 엑셀을 계속 여시게 되는 이유가
여기 하나입니다.

기술적으로는 필요한 것이 이미 다 있습니다(정산 금액·거래처 결제일·
payments 테이블·등록 함수·권한). 없는 것은 **"이 달 정산을 청구로 만든다"는
버튼 하나와, 그 버튼을 눌렀을 때의 방침**입니다 — 그 방침은 §10 에 적었습니다.
