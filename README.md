...
│   ├── BottomSheet.tsx   # 아래에서 올라오는 바텀시트 (프레임 내부 contained)
│   ├── FramePortal.tsx   # 폰 프레임(#app-frame) 내부로 오버레이 포탈
...
└── pages/
    ...
    └── DemoSummary.tsx   # 시연용 핵심 요약 (/demo) — 심사관 시연 흐름
```
...
- **6차 (시연 품질 — 폰 프레임·요약 모드)**
  - **데스크톱 폰 프레임** — 데스크톱에서도 중앙에 최대 420px 모바일 앱처럼 표시(부드러운
    그라데이션 배경 + 둥근 프레임 + 그림자). 사이드바 제거, 하단 탭으로 통일. 녹화/시연 최적화.
  - **본문 내부 스크롤 + ScrollToTop 강화** — 프레임 본문(main)이 스크롤 컨테이너이며 경로 변경 시
    `main.scrollTop = 0` + 윈도우 스크롤 폴백으로 항상 최상단에서 시작
  - 모달·바텀시트·토스트를 **프레임 내부에 contained**(FramePortal/absolute)로 정리
