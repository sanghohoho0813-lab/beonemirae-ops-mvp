...
- **글자 크기 3단계** — 기본 / 크게(+20%) / 매우 크게(+40%). 모바일 기본값은 **크게**. 선택값은 기기에 저장되어 새로고침해도 유지됩니다. (대시보드 상단 및 **더보기 → 글자 크기**에서 변경)
- **하단 메뉴** — 모바일은 핵심 5개(대시보드·오늘 일정·거래처·수거 입력·**더보기**)만 노출하고, 자재/미수금/통계/백업·복원/글자 크기/초기화는 **더보기**에 묶어 터치 영역을 넉넉하게(≥56px) 확보했습니다. `safe-area-inset-bottom` 반영.
...
├── components/
│   ├── Layout.tsx        # 헤더 + 사이드/하단 네비게이션(5개+더보기)
